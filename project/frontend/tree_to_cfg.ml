...
type env =
  { env_vars: var StringMap.t;           (* visible variables in scope, by name *)
    ...
    env_gotos_dst: node list;            (* gotos destination nodes *)
    env_loops_heads: node list;          (* loops head nodes *)
  }
...
let rec stat (env:env) (entry:node) (exit:node) (s:stat) : env =
  ...
  | AST_while ((expr,_),(s1,x1)) ->
    ...
    let env_loops_heads = entry1 :: env2.env_loops_heads in
    { env2 with
      env_break = env1.env_break;
      env_loops_heads }
  ...
  | AST_for (init,expr,incr,(s1,x1)) ->
    ...
    let env_loops_heads = head1 :: env4.env_loops_heads in
    { env4 with
      env_break = env3.env_break;
      env_loops_heads }
  ...
  let env_gotos_dst =
    List.fold_left
      (fun acc (src,(id,x)) ->
         ...
         dst :: acc
      ) env4.env_gotos_dst env4.env_gotos in
...
let prog ((t,x):toplevel list ext) : cfg =
  ...
  { cfg_vars = vars;
    ...
    cfg_jumps_dst = env.env_gotos_dst;
    cfg_loops_heads = env.env_loops_heads;
  }
